...
class TestPairing:
    def test_paired_start_end_merged(self):
        trace = from_openclaw_jsonl(
            [
                _line({"type": "llm.request", "span_id": "r1", "name": "plan", "input": "Q"}),
                _line({"type": "llm.response", "span_id": "r1", "output": "A", "tokens": 42}),
            ]
        )
        assert len(trace.nodes) == 1
        n = trace.nodes[0]
    ...
    def test_stray_start_kept_with_error(self):
        trace = from_openclaw_jsonl(
            [
                _line({"type": "llm.request", "span_id": "orphan", "name": "plan", "input": "Q"}),
            ]
        )
        assert len(trace.nodes) == 1
...
class TestEndToEnd:
    ...
    def test_pre_parsed_dicts_accepted(self):
        events = [
            {"type": "llm.response", "span_id": "s1", "name": "plan", "input": "Q", "output": "A"},
        ]
